...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 flex w-full items-center justify-between border-b-2 border-move-blue bg-neutral-950 px-4 py-3 font-bold uppercase tracking-tighter text-move-blue sm:px-6 sm:py-4">
      <Link to="/move/schedules" className="text-xl font-black italic text-move-blue sm:text-2xl">MOVE</Link>
        ...
        {links.map((link) => (
          <Link
            ...
            className={cn(
              ...
              location.pathname === link.path ? 'border-b-4 border-move-pink text-move-pink' : 'text-neutral-400 hover:bg-move-blue hover:text-white'
            )}
          >
            {link.name}
          </Link>
